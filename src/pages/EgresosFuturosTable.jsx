...
export const EgresosFuturosTable = ({ onEdit, onView }) => {
    // Estados principales
    const [todasLasDeudas, setTodasLasDeudas] = useState([]); // Datos originales del servidor
    const [tiposDeDeuda, setTiposDeDeuda] = useState([]); // Tipos de deuda para el filtro
    ...
    const [filtroTipoDeuda, setFiltroTipoDeuda] = useState('');
    ...
    // Función para cargar todos los egresos futuros
    const cargarTodasLasDeudas = async () => {
        setIsLoading(true);
        setError('');

        try {
            const response = await axios.get(`${apiUrl}/deuda-futura`);
            setTodasLasDeudas(response.data.data || response.data);
        } catch (error) {
            console.error('Error al cargar deudas futuras:', error);
            setError('Error al cargar los registros de deudas futuras');
        } finally {
            setIsLoading(false);
        }
    };

    // Función para cargar los tipos de deuda (para el select de filtro)
    const cargarTiposDeDeuda = async () => {
        try {
            const response = await axios.get(`${apiUrl}/tipo-deuda-futura`);
            setTiposDeDeuda(response.data.data || response.data);
        } catch (error) {
            console.error('Error al cargar tipos de deuda futura:', error);
        }
    };
    ...
    const datosFiltrados = useMemo(() => {
        let datosFiltrados = [...todasLasDeudas];

        // Aplicar filtro de fecha desde
        if (filtroFechaDesde) {
            datosFiltrados = datosFiltrados.filter(deuda => 
                deuda.registroFinancieroDiario?.fecha >= filtroFechaDesde
            );
        }
        ...
        if (filtroFechaHasta) {
            datosFiltrados = datosFiltrados.filter(deuda => 
                deuda.registroFinancieroDiario?.fecha <= filtroFechaHasta
            );
        }
        
        // Aplicar filtro de tipo de deuda
        if (filtroTipoDeuda) {
            datosFiltrados = datosFiltrados.filter(deuda =>
                deuda.tipoDeudaFutura?.nombre === filtroTipoDeuda
            );
        }
        ...
        if (filtroUsuario.trim()) {
            datosFiltrados = datosFiltrados.filter(deuda => 
                deuda.usuario?.nombre?.toLowerCase().includes(filtroUsuario.toLowerCase())
            );
        }

        return datosFiltrados;
    }, [todasLasDeudas, filtroFechaDesde, filtroFechaHasta, filtroTipoDeuda, filtroUsuario]);
        ...
        return datosParaOrdenar.sort((a, b) => {
            ...
            switch (ordenarPor) {
                ...
                case 'monto':
                    ...
                    break;
                case 'tipoDeuda':
                    valorA = a.tipoDeudaFutura?.nombre || '';
                    valorB = b.tipoDeudaFutura?.nombre || '';
                    break;
                case 'usuario':
    ...
    useEffect(() => {
        setPaginaActual(1);
    }, [filtroFechaDesde, filtroFechaHasta, filtroTipoDeuda, filtroUsuario]);
    ...
    const exportarDatos = () => {
        try {
            ...
            const csvContent = "data:text/csv;charset=utf-8," + 
                "ID,Monto,Tipo de Deuda,Fecha de Vencimiento,Usuario\n" +
                datosParaExportar.map(deuda => 
                    `${deuda.id},` +
                    `${deuda.monto},` +
                    `"${deuda.tipoDeudaFutura?.nombre || 'N/A'}",` +
                    `${formatearFecha(deuda.registroFinancieroDiario?.fecha)},` +
                    `"${deuda.usuario?.nombre || 'N/A'}"`
                ).join("\n");

            const encodedUri = encodeURI(csvContent);
            const link = document.createElement("a");
            link.setAttribute("href", encodedUri);
            link.setAttribute("download", `egresos-futuros_${new Date().toISOString().split('T')[0]}.csv`);
            ...
            <div className="max-w-7xl mx-auto">
                ...
                <div className="mb-8">
                    <div className="flex items-center justify-between">
                        <div>
                            <h1 className="text-3xl font-bold text-gray-900">Egresos Futuros</h1>
                            <p className="text-gray-600 mt-1">Gestión de deudas futuras</p>
                        </div>
                ...
                {mostrarFiltros && (
                    <div className="bg-white rounded-xl shadow-md p-6 mb-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4">
                            ...
                            <div>
                                <label className="block text-sm font-medium text-gray-700 mb-2">Tipo de Deuda</label>
                                <select
                                    value={filtroTipoDeuda}
                                    onChange={(e) => setFiltroTipoDeuda(e.target.value)}
                                    className="w-full px-3 py-2 border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500"
                                >
                                    <option value="">Todos</option>
                                    {tiposDeDeuda.map((tipo) => (
                    ...
                    <div className="mb-6 bg-white rounded-xl shadow-md p-4">
                        <div className="flex items-center justify-between text-sm text-gray-600">
                            <span>
                                ...
                                {totalRegistros !== todasLasDeudas.length && 
                                    ` (filtrado de ${todasLasDeudas.length} totales)`
                                }
                            </span>
                            <span>
                ...
                <div className="bg-white rounded-xl shadow-md overflow-hidden">
                    ...
                    <div className="px-6 py-4 border-b border-gray-200 bg-gray-50">
                        <div className="flex items-center justify-between">
                            <div className="flex items-center space-x-4">
                                <h2 className="text-lg font-semibold text-gray-900">Deudas Futuras</h2>
                    ...
                    {isLoading ? (
                        ...
                        <div className="text-center py-12">
                            ...
                            <h3 className="text-lg font-medium text-gray-900 mb-2">No hay egresos futuros</h3>
                            <p className="text-gray-500">
                                {todasLasDeudas.length === 0 
                                    ? "No se encontraron egresos futuros"
                                    : "No se encontraron egresos futuros con los filtros aplicados"
                                }
                            </p>
                        </div>
                    ) : (
                                ...
                                <table className="min-w-full divide-y divide-gray-200">
                                    <thead className="bg-gray-50">
                                        <tr>
                                            ...
                                            <th 
                                                className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                                                onClick={() => handleOrdenar('tipoDeuda')}
                                            >
                                                <div className="flex items-center space-x-1">
                                                    <span>Tipo</span>
                                                    {renderIconoOrden('tipoDeuda')}
                                                </div>
                                            </th>
                                            <th 
                                                className="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider cursor-pointer hover:bg-gray-100"
                                                ...
                                                <td className="px-6 py-4 whitespace-nowrap">
                                                    <div className="flex flex-col">
                                                        ...
                                                        <div className="text-xs text-gray-500">
                                                            {obtenerDiaDeLaSemana(deuda.registroFinancieroDiario?.fecha)}
                                                        </div>
                                                    </div>
                                                </td>
                                                <td className="px-6 py-4 whitespace-nowrap">
